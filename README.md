...
* **Sistemas Legados**:
    ...
    * Legado B (SQL Server)
    * Legado C (Oracle)
* **Fluxo de Captura**: Sistemas Legados -> **Debezium (Connector CDC)** *(via CDC - captura de mudanças)*.
    * *Descrição do Debezium:* Conecta aos bancos legados e captura as alterações.
* **Mensageria**: Debezium -> **Kafka (Cluster)**.
...
* **Interação com o Cliente**: **Usuário (Cliente)** -> **API** *(via HTTPS/REST)*.
